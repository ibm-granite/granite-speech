...
By default, the smoke downloads the official
`ibm-granite/granite-speech-4.1-2b` `multilingual_sample.wav` fixture from Hugging Face. The audio
fixture is pinned to a known snapshot and SHA-256. Run:
...
asserts that it resolves to the default `llama.cpp` backend, transcribes the fixture, requires at
least one segment, and requires no package or transcription warnings. Set
`GRANITE_SPEECH_SMOKE_EXPECTED_TEXT` to additionally compare the output with a known transcript.
...
- `GRANITE_SPEECH_SMOKE_AUDIO`: local WAV or FLAC fixture to use instead of the Hugging Face sample.
- `GRANITE_SPEECH_SMOKE_AUDIO_REVISION`: Hugging Face revision for the default sample.
- `GRANITE_SPEECH_SMOKE_AUDIO_SHA256`: expected SHA-256 for the default sample.
...
- The default audio fixture is approximately 1.6 MB.
